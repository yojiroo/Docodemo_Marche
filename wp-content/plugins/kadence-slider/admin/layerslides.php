...
function ksp_admin_output_Slide($slider, $slide, $edit) {
	...
	$slider_background_video_ratio_options = array(
		'1.777777778' => array(__('16 / 9', 'kadence-slider'), false),
		'1.333333333' => array(__('4 / 3', 'kadence-slider'), false),
		'1.5' => array(__('3 / 2', 'kadence-slider'), false),
		);
	$rand = rand(1, 999);
	?>
										...
										<div class="ksp-content">
											...
											<?php
												...
												foreach($slider_background_video_ratio_options as $key => $value) {
													echo '<option value="' . esc_attr($key) . '"';
													if(($void && $key == "1.777777778") || (!$void && $slide->background_type_video_ratio == $key)) {
														echo ' selected';
													}
													echo '>' . $value[0] . '</option>';
												}
