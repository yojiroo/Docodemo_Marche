var ktwc_settings = (function($, window, document) {
    $( "#kt_woo_new_field_form" ).dialog({
		...
		buttons: [{
			text: "Add New Field",
			click: function() {
				var result = kt_woo_add_new_row( this );
				if(result){
					$( this ).dialog( "close" );
				}
			}
	...
	$( "#kt_woo_edit_field_form" ).dialog({
		...
		buttons: [{
			text: "Edit Field",
			click: function() {
				var result = kt_woo_update_row( this );
				if(result){
					$( this ).dialog( "close" );
				}
			}
	...
	$('select.kt-enhanced-multi-select').select2({
	...
	$('#kt_checkout_fields tbody').sortable({
		items:'tr',
		cursor:'move',
		axis:'y',
		handle: 'td.sort',
	...
	$("#kt_checkout_fields tbody").on("sortstart", function( event, ui ){
		ui.item.css('background-color','#f6f6f6');										
	});
	$("#kt_checkout_fields tbody").on("sortstop", function( event, ui ){
		ui.item.removeAttr('style');
		kt_woo_prepare_field_order_indexes();
	});
	...
	_openNewFieldForm = function openNewFieldForm(tabName){
		...
		var form = $("#kt_woo_new_field_form");
		kt_woo_clear_form(form);
		form.find("input[name=fname]").val(tabName);
		form.find("select[name=ftype]").change();
		form.find("input[name=fclass]").val('form-row-wide');
		
	  	$( "#kt_woo_new_field_form" ).dialog( "open" );
	}
	
	function kt_woo_add_new_row(form){
		...
		var index = $('#kt_checkout_fields tbody tr').size();
		...
		$('#kt_checkout_fields tbody tr:last').after(newRow);
		return true;
	}
	...
	_openEditFieldForm = function openEditFieldForm(elm, rowId){
		...
		var form = $("#kt_woo_edit_field_form");
		...
		form.find("select[name=ftype]").change();
		$( "#kt_woo_edit_field_form" ).dialog( "open" );
	...
	function kt_woo_update_row(form){
		...
		var row = $('#kt_checkout_fields tbody').find('.row_'+rowId);
	...
	_removeSelectedFields = function removeSelectedFields(){
		$('#kt_checkout_fields tbody tr').removeClass('strikeout');
		$('#kt_checkout_fields tbody input:checkbox[name=select_field]:checked').each(function () {
	...
	_enableDisableSelectedFields = function enableDisableSelectedFields(enabled){
		$('#kt_checkout_fields tbody input:checkbox[name=select_field]:checked').each(function () {
			var row = $(this).closest('tr');
			if(enabled == 0){
				if(!row.hasClass("kt-woo-disabled")){
					row.addClass("kt-woo-disabled");
				}
			}else{
				row.removeClass("kt-woo-disabled");				
			}
	...
	function kt_woo_clear_form( form ){
	...
	function kt_woo_prepare_field_order_indexes() {
		$('#kt_checkout_fields tbody tr').each(function(index, el){
			$('input.f_order', el).val( parseInt( $(el).index('#kt_checkout_fields tbody tr') ) );
		});
	};
	...
	_selectAllCheckoutFields = function selectAllCheckoutFields(elm){
		...
		$('#kt_checkout_fields tbody input:checkbox[name=select_field]').prop('checked', checkAll);
	}
