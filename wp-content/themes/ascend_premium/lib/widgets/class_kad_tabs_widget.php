...
class kad_tabs_content_widget extends WP_Widget{
    ...
    public function __construct() {
        $widget_ops = array('classname' => 'kadence_tabs_content_widget', 'description' => __('Adds tabs that can hold widgets.', 'ascend'));
        parent::__construct('kadence_tabs_content_widget', __('Ascend: Tabs & Accordions', 'ascend'), $widget_ops);
    }
       ...
       public function widget($args, $instance){ 
        ...
        $instance['style'] 		= ! empty( $instance['style'] ) ? $instance['style'] : 'kt-tabs-style1';
        ...
        if('accordion' == $instance['display']) {
			...
			<div class="kadence-tabs-container">
				...
				if ( ! empty( $items ) ) :
						$items[0]['active'] = true; // First tab should be active.
					?>
					<ul class="nav kt-tabs kt-sc-tabs <?php echo esc_attr($instance['style']);?>" role="tablist">
						<?php foreach ( $items as $item ) : ?>
    ...
    function sort_by_id( $a, $b ) {
  	...
  	public function form($instance){
	    	...
	    	$style = ! empty( $instance['style'] ) ? $instance['style'] : 'kt-tabs-style1';
			$items        = isset( $instance['items'] ) ? $instance['items'] : array();

  			$display_array = array();
  			$style_array = array();
  			$display_options = array(array("slug" => "tabs", "name" => __('Tabs', 'ascend')), array("slug" => "accordion", "name" => __('Accordion', 'ascend')));
  			$style_options = array(array("slug" => "kt-tabs-style1", "name" => __('Tabs Style 1', 'ascend')), array("slug" => "kt-tabs-style2", "name" => __('Tabs Style 2', 'ascend')));
		    ...
		    foreach ($style_options as $style_option) {
		      	if ($style == $style_option['slug']) { $selected=' selected="selected"';} else { $selected=""; }
		      	$style_array[] = '<option value="' . $style_option['slug'] .'"' . $selected . '>' . $style_option['name'] . '</option>';
		    }
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'display' ) ); ?>"><?php esc_html_e( 'Display as:', 'ascend' ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'style' ) ); ?>"><?php esc_html_e( 'Tab Style:', 'ascend' ); ?></label>
			<select id="<?php echo $this->get_field_id('style'); ?>" style="width:100%; max-width:230px;" name="<?php echo $this->get_field_name('style'); ?>"><?php echo implode('', $style_array);?></select>
		</p>

		<hr>

		<h3><?php esc_html_e( 'Panes:', 'ascend' ); ?></h3>
		...
		<script type="text/template" id="js-kadence-tab-<?php echo esc_attr( $this->current_widget_id ); ?>">
			<div class="kadence-tabs-widget  ui-widget  ui-widget-content  ui-helper-clearfix  ui-corner-all">
				<div class="kadence-tabs-widget-header  ui-widget-header  ui-corner-all">
					...
					<span><?php esc_html_e( 'Pane', 'ascend' ); ?> - </span>
				...
				<div class="kadence-tabs-widget-content">
					<p>
						<label for="<?php echo esc_attr( $this->get_field_id( 'items' ) ); ?>-{{id}}-title"><?php echo __( 'Pane title:', 'ascend' ); ?></label>
					...
					<p>
						...
						<a href="#" class="kadence-remove-tab  js-kadence-remove-tab"><span class="dashicons dashicons-dismiss"></span> <?php echo __( 'Remove pane', 'ascend' ); ?></a>
					</p>
				</div>
			</div>
		</script>
		...
		<div class="kadence-widget-tabs" id="tabs-<?php echo esc_attr( $this->current_widget_id ); ?>">
			...
			<p>
				<a href="#" class="button  js-kadence-add-tab"><?php echo  __( 'Add new pane', 'ascend' ); ?></a>
			</p>
		</div>
