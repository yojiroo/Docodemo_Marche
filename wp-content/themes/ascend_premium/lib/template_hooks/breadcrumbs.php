...
function ascend_display_testimonial_breadcrumbs() {
  global $ascend;
   if(isset($ascend['show_breadcrumbs_testimonial'])) {
  if($ascend['show_breadcrumbs_testimonial'] == 1 ) {$showbreadcrumbs = true;} else { $showbreadcrumbs = false;}
} else {$showbreadcrumbs = true;}
  return $showbreadcrumbs;
}
function ascend_display_staff_breadcrumbs() {
  global $ascend;
   if(isset($ascend['show_breadcrumbs_staff'])) {
  if($ascend['show_breadcrumbs_staff'] == 1 ) {$showbreadcrumbs = true;} else { $showbreadcrumbs = false;}
} else {$showbreadcrumbs = true;}
  return $showbreadcrumbs;
}
...
function ascend_breadcrumbs( $color = null ) {
	...
	$homelink 			= home_url('/');
	$wrap_before    	= '<div id="kadbreadcrumbs"><div class="kt-breadcrumb-container container" style="'.esc_attr( $color_style ).'">';
	$wrap_after     	= '</div></div>';
	...
	$link_in_after  	= '</span>';
	$link           	= $link_before . '<a href="%1$s" itemprop="url" style="'.esc_attr( $color_style ).'">' . $link_in_before . '%2$s' . $link_in_after . '</a>' . $link_after;
	$before 			= '<span class="kad-breadcurrent">';
 	$after 				= '</span>';
 	$home_link      	= $link_before . '<a href="' . esc_url($homelink) . '" itemprop="url" class="kad-bc-home" style="'.esc_attr( $color_style ).'">' . $link_in_before . esc_html($home) . $link_in_after . '</a>' . $link_after . $sep;
 	$shop_bread = '';
    ...
    echo $wrap_before . $home_link;
    	...
    	if ( is_category() ) {
      		...
      		echo $before . ascend_title_archive() . $after;
            ...
            // TRIBE
          	...
          	// STAFF
              	...
              	if( !empty($ascend['staff_link']) ) { 
                    $parentpagelink = get_page_link($ascend['staff_link']);
                    $parenttitle = get_the_title($ascend['staff_link']);
                    echo sprintf($link, esc_url($parentpagelink), esc_html($parenttitle)) . $sep;
                } 
               	$main_term = '';
              	// check if yoast category set and there is a primary
                ...
                // TESTIMONIAL
              	...
              	if( !empty($ascend['testimonial_link']) ) { 
                    $parentpagelink = get_page_link($ascend['testimonial_link']);
                    $parenttitle = get_the_title($ascend['testimonial_link']);
                    echo sprintf($link, esc_url($parentpagelink), esc_html($parenttitle)) . $sep;
                } 
               	$main_term = '';
              	// check if yoast category set and there is a primary
