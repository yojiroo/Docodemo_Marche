...
<div class="eventorganiser-event-meta">
	...
	<!-- Event details -->
	<h4><?php _e( 'Event Details', 'ascend' ); ?></h4>
	...
	<?php if ( eo_recurs() ) :?>
		...
		<?php if ( $next ) : ?>
			...
			<p>
				<span> <?php echo __( 'This is a reoccurring event.', 'ascend'); ?></span>
				<?php printf( __( 'It is next occurring on %1$s', 'ascend' ), $next );?>
			</p>

		<?php else : ?>
			<!-- Otherwise the event has finished (no more occurrences) -->
			<?php printf( '<p>' . __( 'This event finished on %s', 'ascend' ) . '</p>', eo_get_schedule_last( 'd F Y', '' ) );?>
		<?php endif; ?>
	<?php endif; ?>
	...
	<ul class="eo-event-meta">
		...
		<?php if ( ! eo_recurs() ) { ?>
			<!-- Single event -->
			<li><strong><?php esc_html_e( 'Date', 'ascend' );?>:</strong> <?php echo eo_format_event_occurrence();?></li>
		<?php } ?>
		...
		<?php if ( get_the_terms( get_the_ID(), 'event-category' ) && ! is_wp_error( get_the_terms( get_the_ID(), 'event-category' ) ) ) { ?>
			<li><strong><?php esc_html_e( 'Categories', 'ascend' ); ?>:</strong> <?php echo get_the_term_list( get_the_ID(),'event-category', '', ', ', '' ); ?></li>
		<?php } ?>

		<?php if ( get_the_terms( get_the_ID(), 'event-tag' ) && ! is_wp_error( get_the_terms( get_the_ID(), 'event-tag' ) ) ) { ?>
			<li><strong><?php esc_html_e( 'Tags', 'ascend' ); ?>:</strong> <?php echo get_the_term_list( get_the_ID(), 'event-tag', '', ', ', '' ); ?></li>
		<?php } ?>
		...
		<?php if ( eo_recurs() ) {
				...
				if ( $upcoming->have_posts() ) : ?>
					...
					<li><strong><?php _e( 'Upcoming Dates', 'ascend' ); ?>:</strong>
