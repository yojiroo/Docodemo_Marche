	...
	<table class="variations" cellspacing="0">
		<tbody>
			<?php foreach ( $attributes as $attribute_name => $options ) : ?>
				<tr>
					<?php if(isset($ascend['product_radio']) && $ascend['product_radio'] == 1) { ?>

						<td class="product_label"><label for="<?php echo sanitize_title( $attribute_name ); ?>"><?php echo esc_html(wc_attribute_label( $attribute_name )); ?> </label></td>
						 ...
						 <?php  $selected = isset( $_REQUEST[ 'attribute_' . sanitize_title( $attribute_name ) ] ) ? wc_clean( $_REQUEST[ 'attribute_' . sanitize_title( $attribute_name ) ] ) : $product->get_variation_default_attribute( $attribute_name );
						 	...
						 	echo end( $attribute_keys ) === $attribute_name ? apply_filters( 'woocommerce_reset_variations_link', '<a class="reset_variations" style="visibility: hidden;" href="#">' . esc_html($cleartext) . '</a>' ) : '';
						 ?>
					...
					<?php
						$selected = isset( $_REQUEST[ 'attribute_' . sanitize_title( $attribute_name ) ] ) ? wc_clean( $_REQUEST[ 'attribute_' . sanitize_title( $attribute_name ) ] ) : $product->get_variation_default_attribute( $attribute_name );
								...
								echo end( $attribute_keys ) === $attribute_name ? apply_filters( 'woocommerce_reset_variations_link', '<a class="reset_variations" href="#">' . esc_html($cleartext) . '</a>' ) : '';
